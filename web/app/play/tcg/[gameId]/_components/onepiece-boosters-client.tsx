...
import { ONEPIECE_PACK_TYPES, TCG_GAMES } from "@shared/types";
import {
  ONEPIECE_BASE_SET,
...
import {
  ...
  ONEPIECE_RARITY_LABEL,
} from "./onepiece-card-visuals";
import { formatPackRate, onepiecePackRate } from "@shared/tcg-pack-odds";

type ConnStatus = "connecting" | "connected" | "disconnected";

export function OnePieceBoostersClient({
  profile,
  gameId,
}: {
  ...
  const [detailPack, setDetailPack] = useState<OnePiecePackTypeId | null>(null);
  ...
  const packs = Object.values(ONEPIECE_PACK_TYPES);
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/40 p-4 backdrop-blur-sm">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-400">
                {game.packSize} cartes par paquet · slot final : R/SR garanti +
                petite chance de SEC, SP, Leader ou TR (≪ 5%).
              </p>
            </div>
...
function BoosterDetailModal({
  ...
  packId: OnePiecePackTypeId;
  onClose: () => void;
  onZoomCard: (card: OnePieceCardData) => void;
}) {
  const pack = ONEPIECE_PACK_TYPES[packId];
  // Filtre identique à getOnePieceThemedPool côté serveur.
  const cards = ONEPIECE_BASE_SET.filter(
    (c) => c.pack === packId || c.extraPacks?.includes(packId),
  );
  // Du plus commun au plus rare.
  const RARITY_ORDER: OnePieceRarity[] = [
    "c",
    "uc",
    "r",
    "sr",
  ...
  const grouped: Partial<Record<OnePieceRarity, OnePieceCardData[]>> = {};
  for (const c of cards) {
    if (!grouped[c.rarity]) grouped[c.rarity] = [];
    grouped[c.rarity]!.push(c);
  }
  for (const r of RARITY_ORDER) {
    grouped[r]?.sort((a, b) => a.id.localeCompare(b.id));
  }
      ...
      >
        <header className="flex shrink-0 items-center justify-between border-b border-white/10 px-5 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-lg font-bold ${pack.accent}`}>
                {pack.name}
              </h2>
                ...
                <div className="mb-2 flex items-baseline gap-2 border-b border-white/5 pb-1">
                  <span
                    className={`rounded px-2 py-0.5 text-xs font-bold ${
                      ONEPIECE_RARITY_COLOR[rarity]
                    }`}
                  >
                    {ONEPIECE_RARITY_GLYPH[rarity]}
                  </span>
                  <span className="text-sm font-semibold text-zinc-200">
                    {ONEPIECE_RARITY_LABEL[rarity]}
                  </span>
                  <span className="text-xs text-zinc-500">
                    · {list.length} cartes ·{" "}
                    <span className="text-zinc-400">
                      {formatPackRate(onepiecePackRate(rarity))} par pack
                    </span>
                  </span>
                </div>
        ...
        <footer className="shrink-0 border-t border-white/10 px-5 py-2 text-center text-[11px] text-zinc-500">
          ...
          {" "}
          <span className="text-zinc-400">
            les SEC / SP / TR / Leaders sont très rares (≪ 5% chance par pack)
          </span>
        </footer>
      </div>
    </div>
...
function PackRevealOverlay({
  ...
  pack: TcgPackResult;
  cardById: Map<string, OnePieceCardData>;
  onDone: () => void;
}) {
  const [revealedIdx, setRevealedIdx] = useState<number>(-1);
  const cards = pack.cards
    .map((id) => cardById.get(id))
    .filter((c): c is OnePieceCardData => !!c);
...
function RevealCard({
  ...
  const accent =
    ONEPIECE_RARITY_COLOR[card.rarity] ?? ONEPIECE_RARITY_COLOR["c"];
  return (
    <button
    ...
    >
      ...
      >
        <div
          className="absolute inset-0 flex items-center justify-center rounded-xl border border-rose-400/40 bg-gradient-to-br from-rose-700 to-rose-950 shadow-xl"
          style={{ backfaceVisibility: "hidden" }}
        >
          <div className="text-center">
            <div className="text-3xl">🃏</div>
            <div className="mt-1 text-[10px] uppercase tracking-widest text-rose-200/70">
              {clickable ? "Cliquer" : ""}
            </div>
          </div>
        </div>
