...
import { RUNETERRA_PACK_TYPES, TCG_GAMES } from "@shared/types";
import {
  RUNETERRA_BASE_SET,
...
const RARITY_ORDER: RuneterraRarity[] = [
  "Common",
  "Rare",
  "Epic",
  "Champion",
...
export function LorBoostersClient({ profile }: { profile: Profile | null }) {
  ...
  const [detailPack, setDetailPack] = useState<RuneterraPackTypeId | null>(
    null,
  );
  ...
  const packs = Object.values(RUNETERRA_PACK_TYPES);
...
function BoosterDetailModal({
  ...
  packId: RuneterraPackTypeId;
  onClose: () => void;
  onZoomCard: (card: RuneterraCardData) => void;
}) {
  const pack = RUNETERRA_PACK_TYPES[packId];
  const region = pack.region;
  const cards = RUNETERRA_BASE_SET.filter(
    (c) => c.collectible && c.regions.includes(region),
  );
  const grouped: Partial<Record<RuneterraRarity, RuneterraCardData[]>> = {};
    ...
    >
      ...
      >
            ...
            return (
              <section key={rarity} className="mb-6 last:mb-0">
                <div className="mb-2 flex items-baseline gap-2 border-b border-white/5 pb-1">
                  <span
                    className={`rounded border px-2 py-0.5 text-xs font-bold ${LOR_RARITY_COLOR[rarity]}`}
                  >
                ...
                <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
                  {list.map((card) => (
                    <button
                      ...
                      className={`relative aspect-[2/3] overflow-hidden rounded-md border bg-black/40 transition-transform hover:scale-[1.04] hover:ring-2 hover:ring-white/30 ${LOR_RARITY_COLOR[rarity]}`}
                      title={card.name}
                    >
                      {card.image && (
        ...
        <footer className="shrink-0 border-t border-white/10 px-5 py-2 text-center text-[11px] text-zinc-500">
          ...
          {" "}
          <span className="text-zinc-400">
            les Champions sont rares (~17% chance par pack)
          </span>
        </footer>
      </div>
    </div>
...
function PackRevealOverlay({
  ...
  const cards = pack.cards
    .map((id) => cardByCode.get(id))
    .filter((c): c is RuneterraCardData => !!c);
...
function RevealCard({
  ...
  const accent = LOR_RARITY_COLOR[card.rarity] ?? LOR_RARITY_COLOR.Common;
  return (
    <button
